...
-spec update_status(integer(), atom()) ->
    ok
    | {error, term()}.
...
-spec complete_mission(integer(), term()) ->
    ok
    | {error, term()}.
...
-spec fail_mission(integer(), term()) ->
    ok
    | {error, term()}.
...
fail_mission(MissionId, Error) ->
    modify_mission(
        MissionId,
        fun(R) -> R#mission{status = failed, error = Error} end
    ).
...
handle_transaction({atomic, Result}, _Id) ->
    Result;
